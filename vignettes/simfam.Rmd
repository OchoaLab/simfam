...
```{r, fig.width = 7, fig.height = 4, fig.align = 'center'}
# first create list of IDs of interest
# (last generation in this example)
ids_G <- paste0( G, '-', 1:n )
...
plot( obj, cex = 0.7 )
```
Inspection of the top of the table shows that founders that were originally unpaired are now removed (the plot also doesn't report "unplotted" people).
The plot further shows that many individuals in the 2nd generation (childness uncles and aunts of the last generation) were also removed, but nobody in the 3rd generation was removed (as desired).
...
ids_1 <- paste0( '1-', 1:n ) # match `fam` table's IDs
colnames( X_1 ) <- ids_1
```
...
Now we draw genotypes across the pedigree using `draw_geno_fam`!
...
However, this code draws loci independently from each other (there's no LD).
...
rownames( admix_proportions_1 ) <- paste0( '1-', 1:n ) # match `fam` table's IDs
...
In contrast, in the 3rd generation ancestry is much more averaged out, with fewer extremes.
...
K <- 3 # number of intermediate subpopulations
...
fam <- prune_fam( fam, ids_G )
# indexes (really logicals) marking individuals in last generation,
# to subset data (aligned with `fam`)
indexes_G <- fam$id %in% ids_G
...
admix_proportions_1 <- admix_prop_1d_linear( n, K, sigma = 1 )
# add founder names to `admix_proportions_1` (will propagate to other vars of interest)
rownames( admix_proportions_1 ) <- ids_1
# also name subpopulations simply as S1, S2, S3
colnames( admix_proportions_1 ) <- paste0( 'S', 1:K )
...
# admixed founders
X_1 <- draw_all_admix( admix_proportions_1, inbr_subpops, m )$X
# propagate through pedigree
X <- draw_geno_fam( X_1, fam )
# subset last generation
X_G <- X[ , indexes_G ]
...
kinship_total <- kinship_fam( kinship_total_1, fam ) # all generations
kinship_total_G <- kinship_total[ indexes_G, indexes_G ] # last generation only

# kinship due to admixture only
admix_proportions <- admix_fam( admix_proportions_1, fam ) # admix all generations
kinship_admix <- coanc_to_kinship( coanc_admix( admix_proportions, inbr_subpops ) ) # all
kinship_admix_G <- kinship_admix[ indexes_G, indexes_G ] # last generation only
...
for ( g in 1 : G ) {
    # look for this pattern for this generation
    pattern <- paste0( g, '-' )
    # this is the desired fraction
    frac_per_gen[g] <- sum( grepl( pattern, ids ) ) / n
}
