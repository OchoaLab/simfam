# parents are assumed to be data.frames with 2 columns: end, anc
...
recomb_chr <- function( breaks, mat, pat ) {
    ...
    if ( !all( c('end', 'anc') %in% names( mat ) ) )
        stop( '`mat` must have column names "end" and "anc"!' )
    if ( nrow( mat ) < 1 )
        stop( '`mat` must have at least one row!' )
    # validate pat
    if ( !is.data.frame( pat ) )
        stop( '`pat` must be a data.frame (including tibble)' )
    if ( !all( c('end', 'anc') %in% names( pat ) ) )
        stop( '`pat` must have column names "end" and "anc"!' )
    if ( nrow( pat ) < 1 )
        stop( '`pat` must have at least one row!' )
    # make sure chromosomes have the same length
    leng <- mat$end[ nrow( mat ) ]
    if ( pat$end[ nrow( pat ) ] != leng )
    ...
    for ( break_curr in breaks ) {
        ...
        # bottom range first
        while ( p$end[ i_start ] < break_last )
            i_start <- i_start + 1
        # top of range now
        i_end <- i_start
        while ( p$end[ i_end ] < break_curr )
            i_end <- i_end + 1
        ...
        child_tmp$end[ nrow( child_tmp ) ] <- break_curr
